...
def s_dash():
    ...
    rows="".join([f"<tr><td class='p-2 border'>{dict(s).get('name')}</td><td class='p-2 border'><b>{dict(s).get('code')}</b></td><td class='p-2 border text-xs text-blue-600'><a href='/?school={dict(s).get('code')}' class='underline'>/?school={dict(s).get('code')}</a><br>{dict(s).get('sub')}.yourdomain.com</td><td class='p-2 border'>{dict(s).get('admin_user')}</td><td class='p-2 border bg-yellow-200'><b>{dict(s).get('admin_pass')}</b></td><td class='p-2 border flex gap-1'><a href='/school/{dict(s).get('code')}/login' class='bg-black text-white px-2 py-1 rounded text-xs'>Login</a><a href='/superadmin/delete/{dict(s).get('code')}' onclick=\"return confirm('Delete {dict(s).get('name')}? All learners will be deleted! Are you sure?')\" class='bg-red-600 text-white px-2 py-1 rounded text-xs'>🗑️ Delete</a></td></tr>" for s in schools])
    return render_template_string(STYLE+f"""
